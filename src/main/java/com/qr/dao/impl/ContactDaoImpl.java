...
public class ContactDaoImpl implements ContactDao {
	...
	public Contact updateById(Integer id, Contact newContact) {
		try {
			...
			// Committing The Transactions To The Database
			sessionObj.getTransaction().commit();
			logger.info("\nAccount With Id?= " + id + " Is Successfully Updated In The Database!\n");
		} catch (Exception sqlException) {
			if (null != sessionObj.getTransaction()) {
				logger.info("\n.......Transaction Is Being Rolled Back.......\n");
				sessionObj.getTransaction().rollback();
	...
	public Contact deleteById(Integer contactId) {
		try {
			...
			Contact contactObj = (Contact) sessionObj.load(Contact.class, contactId);
			sessionObj.delete(contactObj);

			// Committing The Transactions To The Database
			sessionObj.getTransaction().commit();
			logger.info("\nAccount With Id?= " + contactId + " Is Successfully Deleted From The Database!\n");
			return contactObj;
		} catch (Exception sqlException) {
			if (null != sessionObj.getTransaction()) {
				logger.info("\n.......Transaction Is Being Rolled Back.......\n");
